...
The main objective of this project is to determine the Gaussianity of the time series data obtained from the LIGO detectors. Depending on how far from a Gaussian distribution the characteristics of the obtained data is From the previous section, we can determine whether a signal sample contains a glitch or not. Specifically, the more non-Gaussian the signal, the higher the chance of a glitch being present. have seen how the data needs to.

To achieve this, we employ three statistical tests: the Shapiro-Wilk test, the Kolmogorov-Smirnov test, and the Anderson-Darling test. Each of these tests has its own strengths and weaknesses, and they are used to assess the normality of the data in different ways.
