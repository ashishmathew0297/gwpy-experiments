...
\noindent Some of our statistical methods, particularly the Kolmogorov-Smirnov test, is highly affected by the values of the data. To ensure that the comparison being done in this test is not biased due to the absolute values of the data points, we scale the whitened data to have a mean of 0 and a standard deviation of 1.

\noindent To achieve this the \texttt{StandardScaler} class from the \texttt{sklearn.preprocessing} module is used, which transposes the data to have a zero mean, calculates the standard deviation, and scales the data down such that the standard deviation is 1. The standard score of a sample, which tells us how many standard deviations a data point is from the mean, is given by
