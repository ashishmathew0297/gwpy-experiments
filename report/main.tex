...
\noindent From the experiments conducted, we can conclude that the Shapiro-Wilk test is the most effective at detecting glitches across the full frequency range of 10 Hz to 1024 Hz. It consistently shows the highest true positive rates and F1 scores, a fairly high level of robustness in identifying glitches. However, its overall accuracy being in the 64\% to 74\% range and F1 scores being in the 0.46 to 0.66 range indicate that there is still room for improvement. The Kolmogorov-Smirnov and Anderson-Darling tests perform similarly to one another, with the Kolmogorov-Smirnov test outperforming the Anderson-Darling test, but they are generally less effective than the Shapiro-Wilk test.

\noindent At lower frequencies, the Shapiro-Wilk test is able to detect a larger number of glitches, while at higher frequencies, its performance is slightly reduced but still superior to the other two tests. The Kolmogorov-Smirnov and Anderson-Darling tests show a similar trend, with their true positive rates decreasing at higher frequencies.

\noindent The results also indicate that the effectiveness of these tests is highly dependent on the specific characteristics of each glitch class, including their frequency content and the amount of noise present in the signal. For example, glitches such as Extremely\_Loud and Koi\_Fish are consistently detected across all frequency ranges, while others like Scattered\_Light and Tomte show varying detection rates depending on the frequency range.
