...
\noindent Looking at the Shapiro-Wilk results on a per-glitch level in table \ref{tab:shapiro_full_range_results}, we see that the test's performance varies significantly across each of the glitch classes. This test achieves perfect accuracy and F1 Scores for Extremely\_Loud glitches. In the examples of Blip, Koi\_fish, and Wandering\_Line glitches, the Shapiro-Wilk test outperforms the Kolmogorov-Smrinov test as seen in table \ref{tab:ks_full_range_results} and the Anderson-Darling test results in \ref{tab:ad_full_range_results}. On studying the distributions for a few examples of these glitches, we can clearly see that they diverge from a Gaussian distribution, which has been perfectly captured by all the three tests.

% All struggle: Blip low frequency, Air Compressor, Low Frequency Lines, Low Frequency Burst, Power Line, Paired Doves, Fast Scattering, Scattered Light, Scratchy
% Comparable: Blip, Helix, Light Modulation, Repeating Blips, Tomte, Violin Mode, Whistle
% Shapiro wilks outperforms better: 1080 Lines, 1400 Ripples, Chirp,

% TODO: Add images of the distributions for these glitches
\medskip
\noindent In the case of the 1400\_Ripples, 
