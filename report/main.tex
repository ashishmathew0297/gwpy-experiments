...
This whitened signal can now be used for our statistical testing.
...
\medskip
\noindent To achieve this, we employ three statistical tests: the Shapiro-Wilk test, the Kolmogorov-Smirnov test, and the Anderson-Darling test. Each of these tests have their own strengths and weaknesses, and are used to assess the normality of the data in different ways.

\subsection{Shapiro-Wilks Test}

The Shapiro-Wilks is a parametric test, meaning that it assumes the data follows a specific distribution. The null hypothesis $H_0$ in this case is that the sample data is normally distributed. The original research paper \cite{Shapiro1965} does a good job of explaining how this test works. The idea is to calculate a test statistic $W$ based on the ratio of the best estimate of the sample data's variance its actual variance. The explanation below is adapted from the original paper.

\medskip
\noindent If we were to consider $x_1, x_2, \ldots, x_n$, to be a collection of ordered sample points from a population, ranked smallest to largest, with the sample mean $\bar{x}$ and $a_1, a_2, \ldots, a_n$
