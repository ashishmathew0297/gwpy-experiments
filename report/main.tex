...
\medskip
\noindent All these noise sources collectively produce time series signals which can be treated as stochastic processes with their corresponding joint probability distributions and statistical properties \cite{collaboration_guide_2020}. In gravitational wave data analysis the noise floor of the detectors is approximated to be stationary and Gaussian. In the event of a glitch, the noise has some structure with a high signal-to-noise ratio (SNR), making it \textit{non-Gaussian}. This project aims to develop a tool to test if the distribution of the amplitudes of detector noise is Gaussian or not to help improve the quality of the analysis performed. The null hypothesis ($h_0$) here is that the distribution is Gaussian, indicating that the signal is clean and the alternative hypothesis is that the distribution is non-Gaussian.
