...
\noindent In most cases, the noise in the GW detectors is stationary \cite{collaboration_characterization_2016}, i.e. the characteristics of the noise do not change over time, and the components are uncorrelated, hence keeping it Gaussian. In reality the detector noise may have some non-stationary components, but we ignore these for the time being. Hence, the statistical properties of the noise can be described by the autocorrelation matrix
...
\medskip
\noindent The Wiener-Khinchin theorem \cite{chatfield1989analysis} states that the Fourier transform of this correlation matrix multiplied by two gives us the \textbf{power spectral density} (PSD) $S_n(f)$. The PSD provides a representation of how power is distributed across different frequency bands in a noise signal\cite{Oppenheim_2009, Li:2013lza}, allowing for the identification of dominant frequency components and their corresponding amplitudes. The PSD is calculated as follows:
...
\noindent This satisfies the relation
...
\noindent Here $\langle \cdot \rangle$ represents the average over time, $\tilde{\cdot}$ represents the Fourier transform from \ref{eq:fourier_transform}, and $\cdot^\ast$ represents the complex conjugate.
