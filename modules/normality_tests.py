...
def section_statistics(data: _pd.DataFrame, stat_test: str="Shapiro", sections: int=1) -> list:
    '''
    A function to calculate and display one of the following:
        ...
        if len(y) > 0:
            if stat_test == "Shapiro":
                ...
                if not _math.isnan(section_statistic.statistic):
                    text = f"p={section_statistic.statistic:.3f}"
